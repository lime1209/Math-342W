...
my_list[["B"]] = array(data=1:27, dim=c(3,3,3))
my_list[["C"]] = array(data=1:4^4, dim=c(4,4,4,4))
my_list[["D"]] = array(data=1:5^5, dim=c(5,5,5,5,5))
my_list[["E"]] = array(data=1:6^6, dim=c(6,6,6,6,6,6))
my_list[["F"]] = array(data=1:7^7, dim=c(7,7,7,7,7,7,7))
my_list[["G"]] = array(data=1:8^8, dim=c(8,8,8,8,8,8,8,8))
my_list
```
...
These function returns the estimate of the memory that is used to store the list.
Yes.
...
stored_data=list("Male","Female")
male=list()
male[["Boomer"]]=M_Boomer
male[["GenX"]]=M_GenX
male[["Millennial"]]=M_Millennial
female=list()
female[["Boomer"]]=F_Boomer
female[["GenX"]]=F_GenX
female[["Millennial"]]=F_Millennial
stored_data=list(male,female)
stored_data[["Male"]]=male
stored_data[["Female"]]=female
stored_data
...
n = 14 * 3 * 2 * 3 * 10
?data.frame
X = data.frame(treatment = rep(letters[1:14],length.out=NA,each=n/14), 
               variation = rep(c("A","B","C"),length.out=NA,each=n/(14*3)),
               gender = rep(c("M","F"),length.out=NA,each=n/(14*3*2)),
               generation = rep(c("Boomer","GenX","Millenial"),length.out=NA,each=n/(14*3*2*3)),
               name = rep(unlist(stored_data), times=14*3),
               y=rep(NA,n)
)
X
...
```{r}
Xinput1=array(2:5,dim=c(2,2))
Xinput2=array(6:9,dim=c(2,2))
xtest1=array(c(2.5,3.6,4.2,5.7),dim=c(2,2))
xtest2=array(c(2.7,3.1,4.3,5.7),dim=c(2,2))
y_binary=rep(letters[1:4])
nn_algorithm_predict(Xinput1, y_binary, xtest1)
nn_algorithm_predict(Xinput2, y_binary, xtest2)
...
for (i in 1 : n) {
  yhat = as.numeric(iris$Sepal.Length > iris$Sepal.Length[i])
  num_errors[i] = sum(yhat != y)
}
...
#'
#' @param Xinput      The data frame it takes in
#' @param y_binary    the vector of data labels
#' @param MAX_ITER    Number of epoch
#' @param w           initial value for weight
#'
