...
pacman::p_load(ggplot2)
ggplot()+
  geom_line(aes(x = Nvec, y = log(time_r), col = "time_r"))+
  geom_line(aes(x = Nvec, y = log(time_cpp), col = "time_rcpp"))+
  xlab("ncol")+
...
for(i in n){
  ...
  time_rcpp = c(time_rcpp, mean(microbenchmark(fib_rcpp = nth_fibonacci_cpp(i, .Machine$double.xmin), times = 3, unit = "s")$time))
}
```
...
ggplot() +
  ...
  geom_line(aes(x = 1:n, y = log(time_rcpp), col = "time_rcpp")) +
...
storms %>%
  ...
  slice(1)%>%
  select(name, max_wind, max_pressure, max_ts_diameter, max_hu_diameter)
```
...
storms%>%
  group_by(name)%>%
  summarise(avg_latitude = mean(lat), avg_longitude = mean(long))
...
#since some storms span multiple days, and go into different categories as time processes, I counted the start date of each storm in each category
```
...
storms%>%
  ...
  mutate(week_of_year = week(as.Date(paste(year, month, day, hour,sep = "-"))))
```
...
find_distance = function(lat_1, long_1, lat_2, long_2){
  lat_1 = lat_1*180/pi
  ...
  distance = 6373.0 * c #multiply by radius of earth in km
  distance
}
...
storms %<>% 
  ...
  mutate(dist_from_prev = ifelse(name != lag(name), 0, find_distance(lat, long, lag(lat), lag(long)))) %>% 
...
cor(speed_and_category$avg_ground_speed, speed_and_category$maximum_category)
```
